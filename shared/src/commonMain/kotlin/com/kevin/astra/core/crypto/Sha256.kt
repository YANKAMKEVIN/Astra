...
/**
 * Pure-Kotlin SHA-256 (FIPS 180-4). Kept platform-independent so PKCE code challenges are computed
 * identically on every target and can be unit-tested against known vectors.
 */
object Sha256 {
    ...
    private fun intsToBytes(ints: IntArray): ByteArray {
        ...
        for (i in ints.indices) {
            out[i * 4] = (ints[i] ushr 24).toByte()
            out[i * 4 + 1] = (ints[i] ushr 16).toByte()
            out[i * 4 + 2] = (ints[i] ushr 8).toByte()
            out[i * 4 + 3] = ints[i].toByte()
        }
        return out
    }
}
